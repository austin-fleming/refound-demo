...
export const ProfileCard = ({ data }: { data: Profile }) => (
	<PostCard slug={`/u/${data.username}`}>
		<CardImage src={data.avatarUrl || "/assets/avatar-placeholder.png"} alt={data.username} />
		<CardContent>
			<CardTitle>@{data.username}</CardTitle>
			{data.bio && <CardDescription>{data.bio}</CardDescription>}
		</CardContent>
	</PostCard>
);
