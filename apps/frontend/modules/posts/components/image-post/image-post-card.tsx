...
import { InteractionsBadge } from "./interactions-badge";

export const ImagePostCard = ({
	imagePost: { title, description, postId, imageSource, createdAt, creator, tags, interactions },
}: {
	imagePost: ImagePostAggregate;
}) => {
	console.log({ title, description, postId, imageSource });
	return (
		<article className="flex flex-col gap-2 mb-8 group">
			<div className="relative">
				<figure className="relative w-full pb-[80%] overflow-hidden rounded-md">
					<NextImage src={imageSource} layout="fill" objectFit="cover" alt={title} />
