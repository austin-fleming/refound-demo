...
export const PolyButton = ({label, color = 'primary', style = 'solid', size = 'base', isDisabled, icon, fullWidth, ...rest}: PolyButtonProps) => {
    const {as} = rest

    const classNames = `${S.btn} ${sizeClasses[size]} ${colorClasses[color]} ${isDisabled && S.isDisabled} ${fullWidth && S.fullWidth}`

    if (as === 'link') {
        const {to, isExternal, preload, ...linkRest} = rest

        return <NextLink href={to}><a className={classNames} {...linkRest}>{label}{icon === 'rightArrow' && <RightArrowIcon className="h-[0.85em] w-[0.85em]"/> }</a></NextLink>
    }

    if (as === 'button') {
        const {...buttonRest} = rest

        return <button className={classNames} {...buttonRest}>{label}{icon === 'rightArrow' && <RightArrowIcon className="h-[0.85em] w-[0.85em]"/> }</button>
    }
